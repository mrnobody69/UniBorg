...
async def _(event):
    ...
    if not event.reply_to_msg_id:
       await event.reply("`Syntax: reply to an image with .mms` 'text on top' ; 'text on bottom' ")
       return
    reply_message = await event.get_reply_message() 
    if not reply_message.media:
       await event.reply("```reply to a image/sticker/gif```")
       return
    chat = "@MemeAutobot"
    ...
    if reply_message.sender.bot:
     ...
     hmm = await event.reply("```💫Uploading File💫 ```")
    ...
    async with borg.conversation("@MemeAutobot") as bot_conv:
          ...
          if "Okay..." in response.text:
            await hmm.edit("```⚡File is not image format, converting to image Format⚡```")
            #await hmm.delete()
            thumb = None
            ...
            if event.reply_to_msg_id:
                ...
                if os.path.exists(downloaded_file_name):
                    ...
                    await hmm.edit("File Not Found {}".format(input_str))
            response = await bot_conv.get_response()
            ...
            await hmm.edit("Invalid message type. Plz choose right message type u NIBBA.")
            return
          else: 
